...
def average_slope_intercept(image, lines):
    ...
    for line in lines:
        ...
        parameters = np.polyfit((x1, x2), (y1, y2), 1)  # slope, yintercept
        slope = parameters[0]
...
# temp = cv2.HoughLines(cropped_image, 2, np.pi/180, 100, np.array([]), 0, 0)
...
# cap = cv2.VideoCapture('pics/test2.mp4')
# while(cap.isOpened()):
#     _, frame = cap.read()
...
#     # cv2.imwrite('pics/tutorial.jpg', canny)
#     cv2.imshow('result', combo_image)
#     if cv2.waitKey(1) == ord('q'):  # millisec
#         break
# cap.release()
